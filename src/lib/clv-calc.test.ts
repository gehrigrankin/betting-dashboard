...
describe("clv-calc", () => {
  describe("calculateClv", () => {
    it("grades a beat-closing bet when the line moved in the bettor's favor", () => {
      ...
      expect(result!.clvAmount).toBeCloseTo(7.91, 2)
    })
    ...
    it("grades a matched bet when bet odds equal closing odds", () => {
      ...
      expect(result!.clvAmount).toBe(0)
    ...
    it("scales clvAmount with stake", () => {
      const small = calculateClv(-110, -130, 50)
      const large = calculateClv(-110, -130, 200)
      expect(small).not.toBeNull()
      expect(large).not.toBeNull()
      expect(small!.clvAmount).toBeCloseTo(3.95, 2)
      expect(large!.clvAmount).toBeCloseTo(15.81, 2)
    })

    it("handles positive American odds", () => {
      // decimal(150) = 2.5, decimal(130) = 2.3, ratio = 2.5/2.3 - 1 = 0.0869565...
      const result = calculateClv(150, 130, 100)
      expect(result).not.toBeNull()
      expect(result!.grade).toBe("beat-closing")
      expect(result!.clvPercent).toBeCloseTo(8.7, 2)
      expect(result!.clvAmount).toBeCloseTo(8.7, 2)
    })
    ...
    it("returns null for the invalid -100 < odds < 100 band", () => {
      expect(calculateClv(50, -110, 100)).toBeNull()
      expect(calculateClv(-110, -99, 100)).toBeNull()
      expect(calculateClv("+50", -110, 100)).toBeNull()
    })

    it("accepts the -100 and 100 boundary values", () => {
      const result = calculateClv(-100, 100, 100)
      expect(result).not.toBeNull()
      expect(result!.betDecimalOdds).toBe(2)
      expect(result!.closingDecimalOdds).toBe(2)
    })
